...
pro.init = function(config){
    // 第一次调用构造函数，执行 init config 为 {file:'', callback}
    this._super(config);
    // init config
    config = config||{};
...
pro._parseConfig = function(file,options){
    ...
    this._config = new Parser(
        _util.merge(this._logger,{
            config:file
        })
    );
    Parser.inst = this._config;
    if (options) {
        this._config.set('DEV_MODE', options.isDev);
    }
    ...
    _path.setWebRoot(this._config.get('DIR_WEBROOT'));
...
pro._parseLogConfig = function(){
    ...
    var isDev = this._config.get('DEV_MODE');
    _log.logger.on(
        'log',function(event){
            if (event.level!=null){
...
/**
 * out put data logger to server
 *
 * @private
 * @param {Function} done - data logger done
 */
pro._outputDataLogger = function(done){
    var api = this._config.get('DATA_LOG_API');
    if (!api){
        done.call(this);
        return;
    }
    ...
    _log.logger.info('data log to %s', api);
    var opt = require('url').parse(api);
    var mod = opt.protocol.replace(':','');
    var data = JSON.stringify({
        timestamp: +new Date,
        dependencies: _dep.dump(),
        versions: _io.dumpResource()
    });
    opt.method = 'POST';
    opt.headers = {
        'Content-Type': 'application/json',
        'User-Agent': 'NEJ Toolkit UA',
        'Content-Length': Buffer.byteLength(data, 'utf8')
    };
    var req = require(mod).request(opt);
    req.on('error', function (e) {
        _log.logger.warn('error data log for %s', e.message);
        ondone();
    });
    req.end(data,'utf8',function () {
        ondone();
    });
};
/**
...
pro._outputAllFiles = function(){
    ...
    _io.ondone(
        function(){
            ...
            this._outputDataLogger(function () {
                _log.logger.info(
...
pro._afterResPrepared = function(){
    ...
    const isDev = this._config.get('DEV_MODE')
    if (!isDev) {
        this._result.minify(
            this._config.format({
                bags:'OBF_NAME_BAGS',
