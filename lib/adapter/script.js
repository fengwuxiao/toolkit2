 ...
 * @param  {Object}  config.variables   - global variables
 * @param  {Object}  config.sourcemap   - output source map config,{root:'./sm/',uri:'%s'}
 * @return {Object}  parse result
 */
pro.parse = function(config){
    // exception when ast parse
    if (!this._ast){
        return;
    }
    config = config||{};
    ...
    var compressor = uglify.Compressor({
        ...
        global_defs:_util.merge(
            {
                DEBUG:!1,
                CMPT:!!config.compatible
            },
            config.variables
        )
    });
    ...
    this._ast = this._ast.transform(compressor);
...
pro._mergeCodeAndToAST = function(map,codes){
    ...
    Object.keys(map).forEach(
        function(file){
            ...
            try{
                // parse ast, don't pass toplevel when parse ast
                var code = ret.join('\n'),
                    ast = uglify.parse(
                        code,{
                            fromString:!0,
                ...
                // merge ast
                if (!this._ast){
                    // top level, fix redundancy export when nej export
                    this._ast = ast.clone();
                    //this._ast = this._asts[file];
                }else{
                    this._ast.body =
...
pro._mangleAPI = (function(){
    ...
    var _isNodeNeedMangle = function(node){
        ...
        Object.keys(_keyMap).some(function(key){
            if (node instanceof uglify['AST_'+key]){
                // not mangle property name in string
                // obj = {'_a':'aaaaaaaa'}; not mangle _a
                if (key==='ObjectKeyVal'&&!!node.quote){
                    return !0;
                }
                // for mangle node
                ret = !0;
    ...
    return function(level,bags,except){
        ...
        var walker = new uglify.TreeWalker(function(node){
            ...
            //console.log('node type is %s and node name is %s',type,node.name);
 ...
 * @param  {String}  opt.sourcemap  - source map config, eg. {root:'',uri:'%s'}
 * @param  {Boolean} opt.compatible - compatiable for nej
 * @param  {Object}  opt.variables  - global variables
 * @return {Void}
 */
pro._prepareOutput = function(opt){
    // init output code
    this._result.code = {};
    var smap = opt.sourcemap||{},
        root = smap.root,
        template = smap.uri||'%s';
    if (!!root){
    ...
    Object.keys(this._asts).forEach(
        function(file){
            ...
            // check source map
            if (!!root){
                var suri = root+name+'.map';
                code += '\n//# sourceMappingURL='+util.format(template,suri);
